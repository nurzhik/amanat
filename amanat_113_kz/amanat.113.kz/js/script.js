...
$('.select-slide').on("click", function(){
  if($(this).hasClass('active')){
    $(this).removeClass('active');
    $(this).children('.personal_under').slideUp(700);
  }else{
    $(this).addClass('active');
    $(this).children('.personal_under').slideDown(700);
  }
});
...
/*     if( $('.main_slider').hasClass('slick-slider') ){
  ...
  $('.main_btn--change').on("click", function(event){
    var old_car = $(this).parent().find('.car').text();
    $('#old-car').val(old_car);
  });
